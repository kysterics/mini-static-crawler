...
class TqdmMultiProcessPool(object):
    def __init__(self, process_count):
        ...
        # self.cf_exe = concurrent.futures.ProcessPoolExecutor(self.process_count, initializer=worker_init_function)
    ...
    def __exit__(self, *args, **kwargs):
        self.mp_pool.close()
        # self.cf_exe.shutdown()

    def map(self, global_tqdm, tasks, on_error=lambda x: None, on_done=lambda x: None):
        ...
        for operation, args in tasks:
            ...
            # async_results.append(self.cf_exe.submit(task_wrapper, *wrapper_args))
        ...
        if terminate:
            ...
            self.mp_pool.close()
            # self.cf_exe.shutdown(cancel_futures=True)
